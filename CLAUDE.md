...
**说明**: 本代码库**不包含Stage III的训练代码**，仅包含推理代码。Stage III (PS-GRPO)是在线强化学习阶段，训练代码未开源。
...
**为什么需要PS-GRPO？标量过程奖励的失败模式**

论文发现直接使用标量过程奖励进行在线RL会导致两个严重问题：

1. **奖励欺骗 (Reward Hacking)**:
   - 模型快速学习迎合过程正确性的策略
   - 但过程正确性不等于推理质量
   - 测试准确率反而低于vanilla GRPO

2. **长度偏差 (Length Bias)**:
   - PRM训练数据中，错误答案的后续步骤都标记为负
   - 导致PRM对推理后期步骤保守评分
   - 模型倾向于生成更短的响应，减少推理步骤
...
```
R^i = {
  1,       如果答案正确且无drop-moment (δ_i < ρ)
  1-γ,     如果答案正确但有drop-moment (δ_i ≥ ρ)
  0,       如果答案错误
}
```
- γ=0.5: 奖励惩罚强度
- 对被PRM质疑的正确答案施加50%的惩罚

**算法优势**:
- **不依赖标量过程奖励**: 不直接优化过程分数，避免奖励欺骗
- **相对质量比较**: 通过drop-moment识别相对质量，而非绝对分数
- **隐式惩罚机制**: 对被质疑的rollouts施加惩罚，鼓励严谨推理
- **缓解长度偏差**: 关注相对下降而非绝对分数，避免保守评分影响

**实验验证**:
- PRM的BoN选择能力在在线RL过程中保持稳定
- PRM的错误识别能力（drop-moment检测）保持有效
- 虽然标量奖励不可靠，但相对质量判断可信
...
**模型架构** (`UrsaForConditionalGeneration`):
```
视觉编码器 (SAM-B + SigLIP-L)
    ↓
MLP投影器 (2层)
    ↓
语言模型 (Qwen2.5-Math-Instruct)
    ↓
生成输出
```

**推理代码**: `inference/vllm_infer.py`
...
# 使用vLLM加速推理
llm = LLM(model="./URSA-8B", tensor_parallel_size=1)
...
**推理流程**:
1. **图像编码**: 混合视觉塔处理图像 → 视觉特征
2. **特征投影**: MLP投影器映射到语言空间
3. **特征融合**: 将视觉特征插入到文本embedding中
4. **自回归生成**: 语言模型逐token生成推理过程
5. **答案提取**: 使用正则表达式提取最终答案

**答案提取策略** (`extract_answer_try_all_methods`):
- 优先匹配 `\boxed{答案}` 格式
- 匹配 `†Answer:` 标记
- 匹配多种自然语言答案格式
- 按优先级顺序尝试所有模式
...
**模型架构** (`UrsaForTokenClassification`):
```
视觉编码器 (SAM-B + SigLIP-L)
    ↓
MLP投影器 (2层)
...
**推理代码**: `inference/prm_infer_score.py`
...
**为什么需要特殊token？**
- PRM需要对每个推理步骤进行评分
- 特殊token `и` 作为步骤分隔符，标记每个步骤的结束位置
- 模型在每个 `и` token位置输出该步骤的正确性分数

**步骤标记插入** (`replace_specific_plus_minus_with_ki`):
```python
# 原始推理过程
"Step 1: 计算面积... Step 2: 代入公式... †Answer: 42"

# 插入特殊token后
"Step 1: 计算面积... и Step 2: 代入公式... и †Answer: 42 и"
```
...
with torch.no_grad():
    # 获取模型输出logits
    reward = model(**inputs).logits  # shape: [batch, seq_len, 1]
    ...
    # Sigmoid转换为概率分数
    reward = torch.sigmoid(reward).view(-1)  # [0, 1]之间的分数
```

3. **分数聚合**:
```python
# Min策略：取所有步骤的最小分数（最保守）
min_score = torch.min(reward)

# Avg策略：取所有步骤的平均分数
avg_score = torch.mean(reward)
```

**为什么有两种聚合策略？**
- **Min**: 保守策略，只要有一步错误就认为整体质量低
- **Avg**: 平均策略，综合考虑所有步骤的质量
- 论文实验表明两种策略各有优势，取决于应用场景
...
# 对N个候选答案进行评分
for i in range(N):
    min_reward, avg_reward = single_inference(
        processor, model, cuda_device,
        input_prompt=data[i]['input'],
        image=data[i]['image']
    )
    res_min.append(min_reward.item())
    res_avg.append(avg_reward.item())

# 重塑为 [num_questions, N] 的矩阵
avg_scores = torch.tensor(res_avg).view(-1, N)
min_scores = torch.tensor(res_min).view(-1, N)

# 选择分数最高的答案
_, max_indices = torch.max(avg_scores, dim=1)
selected_answers = labels[torch.arange(labels.size(0)), max_indices]
```

**Drop-moment检测**（用于PS-GRPO）:

```python
# 计算相邻步骤的相对下降
for j in range(len(reward) - 1):
    relative_drop = (reward[j] - reward[j+1]) / reward[j]

    # 如果下降超过阈值ρ=0.3，检测到drop-moment
    if relative_drop > 0.3:
        has_drop_moment = True
        break

# 根据drop-moment调整奖励
if outcome_correct and not has_drop_moment:
    final_reward = 1.0
elif outcome_correct and has_drop_moment:
    final_reward = 0.5  # 惩罚系数γ=0.5
else:
    final_reward = 0.0
```
...
image_features = self.aligner(image_outputs)  # MLP投影

# 将图像特征插入到文本embedding中
...
2. **分类头设计**:
```python
# 从最后一层hidden states提取特征
logits = outputs.hidden_states[-1]  # [batch, seq_len, hidden_size]

# Dropout正则化
logits = self.dropout(logits)

# 线性分类头：hidden_size → 1
logits = self.score(logits)  # [batch, seq_len, 1]
```
...
- 使用bfloat16精度减少内存占用
- 批量处理多个样本
- 支持多GPU并行（通过cuda_device和cuda_sum参数）
- 预先计算tag_id避免重复编码
...
for top in [8, 16, 32, 64]:
    torch.save(
        ...
        output_path.replace('.pt', f'_avg_sampling{top}.pt')
    )
```
