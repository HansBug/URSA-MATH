...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Standalone URSA-RM-8B scoring example that emits the per-step PRM and "
            "RL reproduction statistics used in the paper."
        )
    )
    parser.add_argument(
        "--model-path",
...
def extract_answer_try_all_methods(text: str):
    text = text.replace("[UNUSED_TOKEN_145]", "")
    if not text:
        return text
    last_marker_index = text.rfind("†")
    if last_marker_index != -1:
        return text[last_marker_index + 1 :].strip()
    matches = re.findall(
        (
            r"(†Answer:|†Answer|answer:|Answer:|therefore,|so,|Therefore,|So,|Thus,|thus,)"
            r"([\s\S]*?)(?=answer:|Answer:|therefore,|so,|Therefore,|So,|Thus,|thus,|$)"
        ),
        text,
    )
    if matches:
